...
public final class MultiBubbleManager {
    ...
    private void showActivityInternal(String packageName, String activityName) {
        ...
        TextView bubble = createTextBubble(packageName);
    ...
    private int measureActivityWidth(TextView view,int screenWidth){int max=Math.max(dp(120),screenWidth-dp(24));view.measure(View.MeasureSpec.makeMeasureSpec(max,View.MeasureSpec.AT_MOST),View.MeasureSpec.makeMeasureSpec(dp(ACTIVITY_HEIGHT_DP),View.MeasureSpec.EXACTLY);return Math.min(max,Math.max(dp(120),view.getMeasuredWidth()+dp(24)));}
    ...
    private TextView createTextBubble(String packageName){TextView v=new TextView(context);v.setText(packageName);v.setTextColor(Color.WHITE);v.setTextSize(11);v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setGravity(Gravity.CENTER);v.setSingleLine(true);v.setPadding(dp(12),0,dp(12),0);v.setBackground(round(Color.rgb(72,91,205),14));v.setElevation(14f);return v;}
