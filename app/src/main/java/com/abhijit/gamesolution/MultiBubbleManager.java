...
public final class MultiBubbleManager {
    ...
    public void configure(boolean recentEnabled, boolean activityEnabled) {
        ...
        else if (recentBubble == null) recentRemoved = false;
        if (!activityEnabled) { removeActivityBubble(); activityRemoved = true; }
        else if (activityBubble == null) activityRemoved = false;
    ...
    public void stopTracking() { if (trackerHandler != null && tracker != null) trackerHandler.removeCallbacks(tracker); trackerHandler = null; tracker = null; }

    public void removeAllSecondaryBubbles() { stopTracking(); removeRecentBubble(); removeActivityBubble(); hideDeleteZone(); }
    public void removeRecentBubble() { if (recentBubble != null) removeView(recentBubble); recentBubble = null; }
    public void removeActivityBubble() { if (activityBubble != null) removeView(activityBubble); activityBubble = null; }
    ...
    private void updateRecentApp() {
        ...
        if (pkg.equals(lastRecentPackage) && recentBubble != null) return;
        lastRecentPackage = pkg; showRecentAppInternal(pkg);
    }
    ...
    private void updateCurrentActivity() {
        ...
        if (current.packageName.equals(lastActivityPackage) && current.activityName.equals(lastActivityName) && activityBubble != null) return;
        lastActivityPackage = current.packageName; lastActivityName = current.activityName; showActivityInternal(current.packageName, current.activityName);
    }

    private ForegroundActivity findForegroundActivity() {
        UsageStatsManager manager = (UsageStatsManager) context.getSystemService(Context.USAGE_STATS_SERVICE);
        if (manager == null) return null;
        long end = System.currentTimeMillis(); UsageEvents events = manager.queryEvents(end - 60_000L, end); if (events == null) return null;
        UsageEvents.Event event = new UsageEvents.Event(); ForegroundActivity result = null; long latest = -1L;
        while (events.hasNextEvent()) { events.getNextEvent(event); if (event.getEventType() != UsageEvents.Event.ACTIVITY_RESUMED) continue; String pkg = event.getPackageName(), cls = event.getClassName(); if (pkg == null || cls == null || pkg.equals(context.getPackageName())) continue; if (event.getTimeStamp() > latest) { latest = event.getTimeStamp(); result = new ForegroundActivity(pkg, cls); } }
        return result;
    }

    public void showRecentApp(String packageName) { recentRemoved = false; lastRecentPackage = packageName; showRecentAppInternal(packageName); }
    public void showActivity(String packageName, String activityName) { activityRemoved = false; lastActivityPackage = packageName; lastActivityName = activityName; showActivityInternal(packageName, activityName); }
    ...
    private void showRecentAppInternal(String packageName) {
        ...
        ImageView bubble = createIconBubble(packageName); addBubble(bubble, packageName, false, 150); recentBubble = bubble;
    }
    ...
    private void showActivityInternal(String packageName, String activityName) {
        ...
        TextView bubble = createTextBubble(shortActivityName(activityName)); addBubble(bubble, packageName, true, 230); activityBubble = bubble;
    }
    ...
    private void addBubble(View view, String packageName, boolean displayOnly, int yDp) {
        ...
        params.gravity = Gravity.TOP | Gravity.START; params.x = Math.max(0, sw - width - dp(12)); params.y = Math.max(dp(48), dp(yDp));
            ...
            @Override public boolean onTouch(View v, MotionEvent e) {
                switch (e.getActionMasked()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (Math.abs(dx)>dp(6)||Math.abs(dy)>dp(6)) { if (!moved) showDeleteZone(); moved=true; }
                        if (moved) { int sw=context.getResources().getDisplayMetrics().widthPixels, sh=context.getResources().getDisplayMetrics().heightPixels; int w=view.getWidth()>0?view.getWidth():params.width; params.x=clamp(startX+(int)dx,0,sw-w); params.y=clamp(startY+(int)dy,dp(48),sh-params.height-dp(8)); try{windowManager.updateViewLayout(v,params);}catch(Exception ignored){} overDelete=isOverDeleteZone(params.x,params.y,w,params.height); updateDeleteZoneVisual(overDelete); }
                        return true;
                    case MotionEvent.ACTION_UP:
                        if (moved) { hideDeleteZone(); if (overDelete) { if (v==recentBubble) { removeRecentBubble(); recentRemoved=true; } else if (v==activityBubble) { removeActivityBubble(); activityRemoved=true; } } else if (BubbleSettings.isSnapToEdge(context)) snapToEdge(v,params); return true; }
                        if (!displayOnly) bringAppToFront(packageName); return true;
    ...
    private void showDeleteZone() {
        ...
        TextView zone=new TextView(context); zone.setText("×"); zone.setTextColor(Color.WHITE); zone.setTextSize(30); zone.setTypeface(Typeface.DEFAULT,Typeface.BOLD); zone.setGravity(Gravity.CENTER); zone.setBackground(round(Color.rgb(95,31,43),40)); zone.setElevation(18f); zone.setAlpha(0f); zone.setScaleX(.65f); zone.setScaleY(.65f);
        WindowManager.LayoutParams p=new WindowManager.LayoutParams(dp(DELETE_ZONE_SIZE_DP),dp(DELETE_ZONE_SIZE_DP),overlayType(),WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE,PixelFormat.TRANSLUCENT); p.gravity=Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL;p.y=dp(30);deleteZoneView=zone;windowManager.addView(zone,p);zone.animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(180).setInterpolator(new DecelerateInterpolator()).start();
    }
