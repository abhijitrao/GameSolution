...
public class FloatingService extends Service {
    ...
    private void showBubble() {
        ...
        bubble.setOnTouchListener((v, event) -> { switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: float dx = event.getRawX() - downX, dy = event.getRawY() - downY; if (Math.abs(dx) > dp(6) || Math.abs(dy) > dp(6)) dragging = true; params.x = clamp(startX - (int) dx, dp(4), getResources().getDisplayMetrics().widthPixels - dp(62)); params.y = clamp(startY + (int) dy, dp(48), getResources().getDisplayMetrics().heightPixels - dp(72)); windowManager.updateViewLayout(bubble, params); return true;
    ...
    private void openMenu(int x, int y) {
        ...
        cardView.setPadding(dp(18), dp(14), dp(18), dp(12));
        cardView.setBackground(round(navy, 20));
        cardView.setElevation(22f);

        TextView eyebrow = label("CURRENT APP", 11, primary, Typeface.BOLD);
        cardView.addView(eyebrow);

        TextView title = label(shortPackage(targetPackage), 17, text, Typeface.BOLD);
        title.setPadding(0, dp(3), 0, dp(1));
        cardView.addView(title);

        TextView packageName = label(targetPackage, 12, secondary, Typeface.NORMAL);
        packageName.setSingleLine(true);
        packageName.setEllipsize(android.text.TextUtils.TruncateAt.END);
        cardView.addView(packageName, new LinearLayout.LayoutParams(-1, dp(22)));

        LinearLayout restart = option("↻", "Restart App", "Clear task and relaunch", text);
        cardView.addView(restart, rowLp(dp(58), dp(8)));
        ...
        cardView.addView(forceStop, rowLp(dp(58), dp(8)));
        ...
        cardView.addView(close, rowLp(dp(48), 0));
        close.setOnClickListener(v -> removeMenu());

        WindowManager.LayoutParams menuParams = new WindowManager.LayoutParams(
                dp(292), dp(294), overlayType(),
    ...
    private LinearLayout.LayoutParams rowLp(int height, int bottomMargin) {
