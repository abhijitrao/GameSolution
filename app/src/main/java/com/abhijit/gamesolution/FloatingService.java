...
public class FloatingService extends Service {
    ...
    private void openMenu(int bubbleX, int bubbleY) {
        ...
        LinearLayout forceStop = option("■", "Force Stop & Restart", "Open Android App Info", danger); cardView.addView(forceStop, rowLp(dp(56), dp(6))); forceStop.setOnClickListener(v -> { vibrate(25); String pkg = targetPackage; removeMenu(); boolean ok = RestartExecutor.openForceStopPage(this, pkg); if (ok) Toast.makeText(this, "Tap Force stop, then launch the game again.", Toast.LENGTH_LONG).show(); else Toast.makeText(this, "Unable to open App Info for " + pkg, Toast.LENGTH_SHORT).show(); });
